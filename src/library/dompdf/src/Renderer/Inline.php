...
class Inline extends AbstractRenderer
{
    function render(Frame $frame)
    {
        ...
        // [LINE 65 FIX] Kunci perbaikan: Pastikan $x adalah float sebelum penjumlahan
        $x = (float)$x + $left_margin; 

        // Handle the last child
        if (($bg = $style->background_color) !== "transparent") {
            $this->_canvas->filled_rectangle((float)$x + (float)$widths[3], (float)$y + (float)$widths[0], (float)$w, (float)$h, $bg);
        }

        if (($url = $style->background_image) && $url !== "none") {
            $this->_background_image($url, (float)$x + (float)$widths[3], (float)$y + (float)$widths[0], (float)$w, (float)$h, $style);
        }
        ...
        if ($bp["right"]["style"] !== "none" && $bp["right"]["color"] !== "transparent" && $widths[1] > 0) {
            ...
            $this->$method((float)$x + (float)$w, $y, $h, $bp["right"]["color"], $widths, "right"); // FIX: Cast untuk posisi x
        }
    ...
    protected function get_child_size(Frame $frame, bool $do_debug_layout_line): array {
        ...
        foreach ($frame->get_children() as $child) {
            ...
            if (is_string($child_w) && $child_w === 'auto') {
